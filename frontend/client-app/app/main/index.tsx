    ...
    <View className="flex-1 bg-[#F3F7FC] px-5 pt-14">
      ...
      <Pressable
        ...
        <Text className="text-sm text-[#D1D5DB]">
          {profile ? 'Perfil clínico atual' : formattedRecordedAt ?? 'Sem dados recentes'}
        </Text>
        <View className="mt-3 flex-row items-end gap-2">
          <Text className="text-5xl font-extrabold text-white">
            {profile?.glicemiaMgDl ?? latestGlucose?.value ?? '--'}
          </Text>
      ...
      <View className="mt-4 flex-row gap-3">
        <QuickAction label="Dados de saúde" icon="create-outline" onPress={() => router.push('/main/add-info')} />
        <QuickAction label="Meu perfil" icon="heart-outline" onPress={() => router.push('/main/profile')} />
        <QuickAction label="Ver dashboard" icon="pie-chart-outline" onPress={() => router.push('/main/dashboard')} />
      </View>
    </View>
  );
}
